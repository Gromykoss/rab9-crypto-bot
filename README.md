...
- `/testsignal ADDRESS` - manually trigger the same MSF pipeline from inside the configured Telegram group: pairresolve, best-pair selection, and normal-mode pairmakers.
...
The endpoint only accepts `chain: solana` and Solana-style addresses of 32-44 chars. Valid requests run pairresolve, select the best resolved pair when available, run normal-mode pairmakers, and post the compact report into the configured Telegram group.
